 ...
 */
export async function createBookingInTransaction(tx, { requestedById, resourceId, title, purpose, startAt, endAt, purposeCode, acceptedQuote }) {
  const start = new Date(startAt);
  const end = new Date(endAt);
...
export async function createBooking(args) {
  return prisma.$transaction((tx) => createBookingInTransaction(tx, args));
}

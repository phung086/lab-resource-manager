...
router.get("/:id/history", requireAuth, async (req, res, next) => {
  try {
    ...
    if (req.user.role === LAB_STAFF) {
      await assertLabStaffResourceAccess(req.user.id, req.params.id);
      staffHasAccess = true;
    }
    const rawHistory = await getResourceHistory(prisma, req.params.id);
    res.json(projectResourceHistoryForActor(rawHistory, req.user, { staffHasAccess }));
  } catch (error) {
    next(error);
  }
});

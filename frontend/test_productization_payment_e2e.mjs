...
const out = path.resolve("screenshots_temp_productization_review");
...
try {
  ...
  await page
    ...
    .click();
  await page.getByRole("link", { name: "Tiếp tục đến VNPAY Sandbox" }).waitFor();
  ...
  await page
    ...
    .waitFor();
  assert.equal(
    await page.getByRole("link", { name: "Tiếp tục đến VNPAY Sandbox" }).count(),
    0,
  );
  assert.equal(
    await page
  ...
  await page
    ...
    .getByText("Đã kiểm tra dữ liệu hệ thống", { exact: false })
    .waitFor();
  ...
  await page.getByRole("button", { name: "Gửi câu hỏi" }).click();
  await page
    .getByRole("button", { name: /Mở form đặt lịch/ })
    .first()
    .waitFor();
  assert.ok(
    await page

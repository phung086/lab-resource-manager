  ...
  return (
    ...
    >
      <div className="resource-detail-status-line">
        <span className={`resource-status-pill status-${String(resource.operationalStatus).toLowerCase()}`}>{resource.operationalStatus}</span>
        <span className={`resource-status-pill status-${String(resource.availability?.state || "unavailable").toLowerCase()}`}>{resource.availability?.state || "UNAVAILABLE"}</span>
      ...
      <dl className="resource-detail-grid">
        ...
        <Detail label="Chính sách đặt lịch" value={resource.bookingState} />
      ...
      <section className="resource-detail-section" aria-labelledby="resource-schedule-heading">
        ...
        {bookings.length === 0 && maintenance.length === 0 ? <p className="resource-detail-empty">Không có lịch đã lưu trong khoảng đang xem.</p> : (
          <div className="resource-event-list">
            {bookings.map((row: any) => <div key={`booking-${row.id}`}><strong>Booking · {row.status}</strong><span>{formatDate(row.startAt)} → {formatDate(row.endAt)}</span></div>)}
            {maintenance.map((row: any) => <div key={`maintenance-${row.id}`}><strong>{row.kind === "calibration" ? "Hiệu chuẩn" : "Bảo trì"} · {row.status}</strong><span>{row.title} · {formatDate(row.startAt)} → {formatDate(row.endAt)}</span></div>)}
          </div>
        )}
      </section>
...
function historyLabel(event: any) {
  if (event.eventType === "OPERATIONAL_STATUS_CHANGED") return `${event.data?.fromStatus} → ${event.data?.toStatus}${event.data?.reason ? `: ${event.data.reason}` : ""}`;
  if (event.source === "booking") return `Booking · ${event.data?.status}`;
  if (event.source === "maintenance_window") return `${event.data?.title || "Bảo trì"} · ${event.data?.status}`;
